...
when_to_use: '"/memo-note <text>" for quick capture, "/memo-note list" to view inbox, "/memo-note process" to triage.'
model: sonnet
effort: medium
user-invocable: true
...
## I/O
- Input: `/memo-note <text>`, `/memo-note list`, `/memo-note process` commands.
...
3. **PROCESS**: Walk pending notes oldest-first. Per note: [s]ave (invoke memo-save skill, delete), [d]efer (patch status to deferred), [x]delete, [q]uit. Summary on exit. See `references/process.md`.
...
- Single URL in capture → offer `/memo-ingest` redirect.
